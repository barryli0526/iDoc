          ...
          <div class="media-content">
            <div class="content">
              <p style="color: #fff;">
                ...
                <br>
                <span style="color: #B5E6FF;">Barlow Respiratory Hospital</span>
              </p>
            </div>
          </div>
      </article>
    ...
    <ul class="menu-list">
      <li v-for="(item, index) in menu">
        <router-link :to="item.path" :exact="true" :aria-expanded="isExpanded(item) ? 'true' : 'false'" v-if="item.path" @click.native="toggle(index, item)">
          ...
          <span class="icon is-small"><img :src="item.meta.img"/></span>
...
.app-sidebar {
  ...
  z-index: 1024 - 1;
  // background: #FFF;
  background: linear-gradient(0deg, #00AAFF 0%, #4CC3FF 100%);
